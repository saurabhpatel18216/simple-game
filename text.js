...
const drawBoard = () => {
  boxes.forEach((box, index) => {
    let styleString = "";
    if (index < 3) {
      styleString += `border-bottom: 2.5px solid var(--purple);`;
    }
    if (index % 3 === 0) {
      styleString += `border-right: 2.5px solid var(--purple);`;
    }
    if (index % 3 === 2) {
      styleString += `border-left: 2.5px solid var(--purple);`;
    }
    if (index > 5) {
      styleString += `border-top: 2.5px solid var(--purple);`;
    }
...
const hasPlayerWon = (player) => {
  ...
  if (spaces[4] === player) {
    ...
    if (spaces[2] === player && spaces[6] === player) {
        console.log(`${player} wins on the other digonal`);
        return true;
  }
}};
